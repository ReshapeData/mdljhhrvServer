...
salaryServer <- function(input,output,session,dms_token) {
  ...
  shiny::observeEvent(input$btn_salary,{
    ...
    mdljhhrvPkg::stdsalary(token = token,FYear =FYear ,FMonth =FMonth ,FExpenseOrgID =FExpenseOrgID )
    
    
    
    mdljhhrvPkg::srcsalary(token = token,FYear =FYear ,FMonth =FMonth ,FExpenseOrgID =FExpenseOrgID )
    
    
    mdljhhrvPkg::odssalary(token = token,FYear =FYear ,FMonth =FMonth ,FExpenseOrgID =FExpenseOrgID )
